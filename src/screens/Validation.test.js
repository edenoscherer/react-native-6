...
describe('Main Initial', () => {
  ...
  it('Should initialize the client', () => {
    ...
    expect(ApolloBoost).toHaveBeenCalledWith({
  ...
  it('Should parse the get pokemon query', () => {
    ...
    expect(apolloSpy).toHaveBeenCalledWith(
      expect.stringContaining('query pokemons($first: Int!) {'),
    );
    expect(apolloSpy).toHaveBeenCalledWith(
      expect.stringContaining('pokemons(first: $first) {'),
    );
  ...
  it('Should have the pokemon image after loading it', () => {
    ...
    expect(
      instance.findByProps({className: 'pokemon-image'}).props.source.uri,
    ).toBe('photo');
  });
  ...
  it('Should have the pokemon name after loading it', () => {
    ...
    if (instance.findByProps({className: 'pokemon-name'}).props.children.join) {
      expect(
        instance
          .findByProps({className: 'pokemon-name'})
          .props.children.join(''),
      ).toBe('001 - Test');
    } else {
      expect(
        instance.findByProps({className: 'pokemon-name'}).props.children,
      ).toBe('001 - Test');
    }
  });
  ...
  it('Should have the pokemon type after loading it', () => {
    ...
    const instance = renderer.create(<Main />).root;
    expect(
      instance.findAllByProps({className: 'pokemon-type'})[0].props.children,
    ).toBe('testType');
    expect(
      instance.findAllByProps({className: 'pokemon-type'})[2].props.children,
    ).toBe('testType 2');
  });
});
